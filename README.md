...
## Methodology
The core of the analysis utilizes Bayesian Gaussian Mixture Regression (BGMR) to map the joint distribution of six environmental pillars: Relative Humidity ($RHUM$), Cloud Cover ($CLDC$), Atmospheric Pressure ($PRES$), Precipitation ($PRCP$), Snow Depth ($SNWD$), and seasonality. Unlike traditional deterministic models, the BGMR approach accounts for atmospheric uncertainty, allowing for the calculation of Exceedance Probabilities and the identification of distinct "Infrastructure Stress Clusters."Key Innovations
Normalization: Shifted the metric from raw report volume to Intersection-Day Occurrences, eliminating reporting bias and focusing on unique structural failure events.Spatial Accuracy: Developed a custom geocoding pipeline involving automated Python scripts and manual Google Maps verification to ensure precise coordinate mapping for thousands of Toronto intersections.
